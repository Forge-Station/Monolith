...
public sealed partial class DebrisFeaturePlacerSystem : BaseWorldSystem
{
    ...
    /// <summary>
    /// Asteroid debris is regenerated by worldgen. Saving it would persist thousands of throwaway grids.
    /// Ore hauled onto a station, POI, or shuttle still saves with that grid.
    /// </summary>
    private void OnPersistenceSerializable(Entity<MetaDataComponent> ent, ref bool serializable)
    {
        if (serializable && HasComp<OwnedDebrisComponent>(ent))
            serializable = false;
    }
    // Forge-Change-End

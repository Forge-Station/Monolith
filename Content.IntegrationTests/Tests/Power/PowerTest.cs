...
";
        ...
        [Test]
        public async Task ApcReceiverNeedsPowerNoopDoesNotForceRecalculate()
        {
            await using var pair = await PoolManager.GetServerClient();
            var server = pair.Server;
            var entityManager = server.ResolveDependency<IEntityManager>();
            ...
            await server.WaitAssertion(() =>
            {
                var receiverEnt = entityManager.SpawnEntity("ApcPowerReceiverDummy", MapCoordinates.Nullspace);
            ...
            await server.WaitAssertion(() =>
            {
                ...
                receiver.NeedsPower = receiver.NeedsPower;
                Assert.That(receiver.Recalculate, Is.False);

                receiver.NeedsPower = !receiver.NeedsPower;
                Assert.That(receiver.Recalculate, Is.True);
            });
        ...
        [Test]
        public async Task ApcReconnectChurnKeepsPowerTopologyValid()
        {
            await using var pair = await PoolManager.GetServerClient();
            var server = pair.Server;
            var mapManager = server.ResolveDependency<IMapManager>();
            ...
            var extensionCableSystem = entityManager.System<ExtensionCableSystem>();
            var mapSys = entityManager.System<SharedMapSystem>();
            var powerNetSystem = entityManager.System<PowerNetSystem>();
            EntityUid apcExtensionEnt = default;
            EntityUid powerReceiverEnt = default;
            ApcPowerReceiverComponent receiver = default!;
            ...
            await server.WaitAssertion(() =>
            {
                ...
                powerReceiverEnt = entityManager.SpawnEntity("ApcPowerReceiverDummy", grid.Owner.ToCoordinates(0, range - 1));
                receiver = entityManager.GetComponent<ApcPowerReceiverComponent>(powerReceiverEnt);
            ...
            for (var i = 0; i < 6; i++)
            {
                ...
                await server.WaitAssertion(() =>
                {
                    ...
                    powerNetSystem.Validate();
                });

                await server.WaitAssertion(() =>
                {
                    extensionCableSystem.SetProviderTransferRange(apcExtensionEnt, 0);
                    extensionCableSystem.SetReceiverReceptionRange(powerReceiverEnt, 0);
                });

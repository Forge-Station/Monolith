...
public sealed class DiceOfFateSystem : EntitySystem
{
    ...
    public void RollFate(EntityUid user, int value)
    {
        ...
        {
            1 => Gib(user), // работает
            2 => Kill(user), //не убивает почему то кпб, человека убило, скорее всего из за того что выдаеться урон удушьем
            3 => SummonMobs(pos), //Поменять фауну, спавняться только пауки, слизни и карпы и голокарпы, из них атакуют только голокарпы и карпы
            4 => DestroyItems(user), // работает(удаляет всю одежду)
            5 => _polymorph.PolymorphEntity(user, Monkey) != null, // работает
            6 => SlowDown(user), // вроде замедляет но в movenmentspeedmodifercomponent модификатор 1
            7 => StunAndDamage(user), // работает
            8 => Explode(user),// надо нерфить
            9 => Nothing(user),//_disease.TryInfect(user, _random.Pick(Diseases), out _, force: true),
            10 => Nothing(user),
            11 => SpawnInHand(user, Cookie, pos), //работает
            12 => Heal(user), // работает
            13 => SpawnInHand(user, Cash, pos), // работает
            14 => SpawnInHand(user, Revolver, pos), // работает
            15 => SpawnInHand(user, RandomSpellbook, pos), // работает
            16 => SpawnInHand(user, RandomImplanter, pos), // работает
            17 => SpawnInHand(user, ThiefToolbox, pos), // работает
            18 => FullAccess(user), // работает
            19 => Resist50(user), // не дает резисты кпб, возможно и другим
            20 => Nothing(user),//BecomeWizard(user),
            _ => Nothing(user)
        };

        _admin.Add(LogType.Action, LogImpact.Extreme,
            $"{ToPrettyString(user):user} rolled Dice of Fate and got {value} (success: {ok})"); //логи не работают
    }
    ...
    {
        _stun.TryKnockdown(user, TimeSpan.FromSeconds(30), true);

...
namespace Content.Client.Paper.UI
{
    ...
    public sealed partial class PaperWindow : BaseWindow
    {
        ...
        public void Populate(PaperComponent.PaperBoundUserInterfaceState state)
        {
            ...
            // Keep a hidden label for font metrics in Draw(). Drawings are
            // sibling TextureRects so RobustToolbox rich-text is untouched.
            WrittenTextLabel.SetMessage(msg, _allowedTags, DefaultTextColor);
            RebuildWrittenContent(state.Text, state.StampedBy.Count > 0);

            if (isEditing)
            {
                WrittenContent.Visible = false;
        ...
        private void RunOnSaved()
        {
            ...
            // Forge-Change: store compact pixel-art instead of one [color] tag per pixel
        ...
        private void UpdateFillState(bool keepStatusMessage = false)
        {
            if (MaxInputLength != -1)
            {
                // Forge-Change: count the compressed length so pasted drawings can be saved

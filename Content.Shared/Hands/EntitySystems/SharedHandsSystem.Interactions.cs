...
public abstract partial class SharedHandsSystem : EntitySystem
{
    ...
    {
        if (ent.Comp.ActiveHand == null || ent.Comp.Hands.Count < 2)
            return;

        var currentIndex = ent.Comp.SortedHands.IndexOf(ent.Comp.ActiveHand.Name);

        if (currentIndex == -1)
            return;

        var newActiveIndex = (currentIndex + (reverse ? -1 : 1) + ent.Comp.SortedHands.Count)
                % ent.Comp.SortedHands.Count;

        var nextHand = ent.Comp.SortedHands[newActiveIndex];

        TrySetActiveHand(ent.Owner, nextHand);

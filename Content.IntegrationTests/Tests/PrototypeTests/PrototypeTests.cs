...
public sealed class PrototypeTests
{
    ...
    public async Task TestAllServerPrototypesAreSerializable()
    {
        ...
        var seriMan = pair.Server.ResolveDependency<ISerializationManager>();
        var context = new PrototypeSaveTest.TestEntityUidContext(seriMan);
    ...
    {
        await using var pair = await PoolManager.GetServerClient();
        var seriMan = pair.Client.ResolveDependency<ISerializationManager>();
        var context = new PrototypeSaveTest.TestEntityUidContext(seriMan);

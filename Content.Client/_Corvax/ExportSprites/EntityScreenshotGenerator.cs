...
public sealed partial class EntityScreenshotGenerator
{
    ...
    private bool TryGetPrototypeIcon(EntityPrototype prototype, out SpriteSpecifier? icon)
    {
        ...
        {
            var node = _serialization.WriteValueAs<DataNode>(entry.Component.GetType(), entry.Component, alwaysWrite: true);
            if (TryExtractSpriteSpecifier(entry.Component.GetType(), node, out icon))
                return true;
        }

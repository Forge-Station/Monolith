...
public sealed partial class ShipyardConsoleMenu : FancyWindow
{
    ...
    [Dependency] private readonly ILocalizationManager _localization = default!; //Forge-Change: add localization manager
    ...
    private List<VesselPrototype?> GetVesselPrototypesFromIds(IEnumerable<string> protoIds)
    {
        ...
        vesselList.Sort((x, y) =>
            string.Compare(VesselDisplayLocale.GetLocalizedName(_localization, x!), VesselDisplayLocale.GetLocalizedName(_localization, y!), StringComparison.CurrentCultureIgnoreCase)); //Forge-Change: add localization manager
        return vesselList;
    }
    ...
    private void AddVesselsToControls(IEnumerable<VesselPrototype?> vessels, string search, bool free, bool canPurchase)
    {
        foreach (var prototype in vessels)
        {
            ...
            if (_engine != null && !prototype!.Engines.Contains(_engine.Value))
                continue;
            // Forge-Change-start: add locale for vessel names
            if (search.Length > 0)
            {
                var displayName = VesselDisplayLocale.GetLocalizedName(_localization, prototype!); //Forge-Change: add localization manager
                if (!prototype!.Name.ToLowerInvariant().Contains(search) &&
                    !displayName.ToLowerInvariant().Contains(search))
                    continue;
            }
            // Forge-Change-end: add locale for vessel names
    ...
    public void UpdateState(ShipyardConsoleInterfaceState state)
    {
        ...
        {
            DeedTitle.Text = Loc.GetString("shipyard-console-registered-none"); // Forge-Change: add locale for vessel names
        }
